...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (path: string) => {
    if (!pathname) return 'text-indigo-100 hover:bg-indigo-700';
      ...
      >
        ...
        <nav className="mt-10">
          ...
          <Link
            ...
            className={`flex items-center px-6 py-2 mt-4 ${isActive('/admin/inspirations')}`}
            onClick={() => setSidebarOpen(false)}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
            </svg>
            <span className="mx-3">Inspirations</span>
          </Link>

          <Link
            href="/admin/promotions"
            className={`flex items-center px-6 py-2 mt-4 ${isActive('/admin/promotions')}`}
            onClick={() => setSidebarOpen(false)}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
