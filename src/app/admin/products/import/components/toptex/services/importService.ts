...
// Fonction pour normaliser une URL d'image
export const normalizeImageUrl = (url: string): string[] => {
  if (!url) return [];
  
  // Diviser les URLs multiples (séparées par |, ; ou ,)
  const urls = url.split(/[|;,]/).filter(Boolean).map(u => u.trim());
  
  return urls.map(singleUrl => {
    // Si l'URL ne commence pas par http:// ou https://, ajouter https://
    if (!singleUrl.match(/^https?:\/\//)) {
      return `https://${singleUrl}`;
    }
    return singleUrl;
  });
};
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < parentProductIds.length; batchIndex += BATCH_SIZE) {
      ...
      await processWithConcurrencyLimit(batchProductIds, MAX_CONCURRENT_OPERATIONS, async (parentProductId) => {
        ...
        let mainImageUrl = firstRow[columnMapping.mainImage] || '';
        const modelImageA = firstRow[columnMapping.modelImageA] || '';
        
        // Si mainImage est vide, utiliser modelImageA comme fallback
        if (!mainImageUrl && modelImageA) {
          console.log(`mainImage est vide pour ${parentProductId}, utilisation de modelImageA comme fallback`);
          mainImageUrl = modelImageA;
        }
        
        // S'assurer que l'URL commence par https://
        if (mainImageUrl && !mainImageUrl.match(/^https?:\/\//)) {
          mainImageUrl = `https://${mainImageUrl}`;
        }
        
        console.log(`Image finale pour ${parentProductId}:`, {
          mainImage: firstRow[columnMapping.mainImage] || 'vide',
          modelImageA: modelImageA || 'vide',
          imageUtilisée: mainImageUrl || 'vide'
        });
        ...
        for (const [colorKey, colorRows] of colorGroups.entries()) {
          try {
            ...
            for (const row of colorRows) {
              ...
              let size = columnMapping.size ? row[columnMapping.size] || 'Unique' : 'Unique';
              
              // Nettoyer la valeur de taille (enlever les apostrophes)
              if (typeof size === 'string') {
                size = size.replace(/['’]/g, ''); // Enlever les apostrophes et les apostrophes typographiques
              }
            ...
            let mainImageRaw = firstColorRow[columnMapping.mainImage];
            let modelImageARaw = firstColorRow[columnMapping.modelImageA];
            let modelImageBRaw = firstColorRow[columnMapping.modelImageB];
            let modelImageCRaw = firstColorRow[columnMapping.modelImageC];
            
            // Si mainImage est vide, utiliser modelImageA comme fallback
            if (!mainImageRaw && modelImageARaw) {
              console.log('mainImage est vide, utilisation de modelImageA comme fallback');
              mainImageRaw = modelImageARaw;
            }
            
            // Ajouter manuellement le préfixe https:// aux URLs non vides
            if (mainImageRaw && typeof mainImageRaw === 'string' && !mainImageRaw.match(/^https?:\/\//)) {
              mainImageRaw = `https://${mainImageRaw}`;
            }
            if (modelImageARaw && typeof modelImageARaw === 'string' && !modelImageARaw.match(/^https?:\/\//)) {
              modelImageARaw = `https://${modelImageARaw}`;
            }
            if (modelImageBRaw && typeof modelImageBRaw === 'string' && !modelImageBRaw.match(/^https?:\/\//)) {
              modelImageBRaw = `https://${modelImageBRaw}`;
            }
            if (modelImageCRaw && typeof modelImageCRaw === 'string' && !modelImageCRaw.match(/^https?:\/\//)) {
              modelImageCRaw = `https://${modelImageCRaw}`;
            }
            
            // Normaliser les URLs d'images (gérer les URLs multiples)
            const mainImages = normalizeImageUrl(mainImageRaw);
            const modelImagesA = normalizeImageUrl(modelImageARaw);
            const modelImagesB = normalizeImageUrl(modelImageBRaw);
            const modelImagesC = normalizeImageUrl(modelImageCRaw);
            
            console.log('URLs d\'images normalisées:', {
              mainImages,
              modelImagesA,
              modelImagesB,
              modelImagesC
            ...
            if (mainImages.length > 0 || modelImagesA.length > 0 || modelImagesB.length > 0 || modelImagesC.length > 0) {
              try {
                // Préparer toutes les images à traiter
                const imagesToProcess: ImageToProcess[] = [];
                
                // Images principales (is_primary = true pour au moins une image par produit)
                if (mainImages.length > 0) {
                  ...
                  mainImages.forEach((mainImageUrl, index) => {
                    console.log(`Traitement de l'image principale ${index + 1}/${mainImages.length} pour ${newProduct.id}, variante ${variantId}:`, {
                      url: mainImageUrl,
                      isPrimary: isPrimary && index === 0, // Seule la première image est principale
                ...
                const allModelImages = [...modelImagesA, ...modelImagesB, ...modelImagesC];
                allModelImages.forEach(modelImageUrl => {
                  imagesToProcess.push({
                    url: modelImageUrl,
                    productId: newProduct.id,
                    variantId: variantId,
                ...
                const imageResults = await processWithConcurrencyLimit(imagesToProcess, MAX_CONCURRENT_IMAGES, async (img) => {
                  try {
                    ...
                    if (!img.url || img.url === '0' || img.url === 'undefined') {
